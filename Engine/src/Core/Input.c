...
uint8_t lc_IsKeyPressed(lcWindow_t *window, int keycode)
{
    int state = glfwGetKey(window->NativeWindow, keycode);
    return state == GLFW_PRESS || state == GLFW_REPEAT;
}

uint8_t lc_IsMouseButtonPressed(lcWindow_t *window, int button)
{
    int state = glfwGetMouseButton(window->NativeWindow, button);
    return state == GLFW_PRESS || state == GLFW_REPEAT;
}

void lc_GetMousePos(lcWindow_t *window, float *output)
{
    double x, y;
